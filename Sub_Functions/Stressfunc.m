function [StressTensor]=Stressfunc(Totalnodes,Nod,EffectiveModulusConcrete,EffectiveModulusSteel,v_concrete,v_steel,G_concrete,G_steel,coordinates,disp,StrainTensor,MaterialFlag)
...
pointsize=1;
figure;
scatter3(coordinates(:,1)+(disp(:,1,1)*10),coordinates(:,2)+(disp(:,2,1)*10),coordinates(:,3)+(disp(:,3,1)*10),pointsize,StressTensor(:,1,1))
title('Stress')
axis equal
colormap jet 
colorbar
